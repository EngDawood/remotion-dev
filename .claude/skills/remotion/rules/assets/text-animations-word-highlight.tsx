	...
	return (
		<span style={{position: 'relative', display: 'inline-block'}}>
			<span style={{
				...
				backgroundColor: color, borderRadius: '0.18em', zIndex: 0,
			}} />
	...
	return (
		<AbsoluteFill style={{backgroundColor: COLOR_BG, alignItems: 'center', justifyContent: 'center', fontFamily}}>
			<div style={{color: COLOR_TEXT, fontSize: FONT_SIZE, fontWeight: FONT_WEIGHT}}>
				{hasHighlight ? (
					<><span>{preText}</span><Highlight word={HIGHLIGHT_WORD} color={COLOR_HIGHLIGHT} delay={HIGHLIGHT_START_FRAME} durationInFrames={HIGHLIGHT_WIPE_DURATION} /><span>{postText}</span></>
				) : (
					<span>{FULL_TEXT}</span>
				)}
			</div>
